...
export async function generateProjectGoalsWithClaude({ description, projectName }) {
  const anthropic = new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY })

  const prompt = `You are a strategic risk advisor generating a goal portfolio for a specific project using the De-Risk Matrix platform.

Project: ${projectName || 'Project'}
Description: ${description}

Generate exactly 7 measurable project goals using the De-Risk Matrix methodology.
...
Rules:
1. Choose goals relevant to this specific project (schedule, cost, scope, quality, team, stakeholder, etc.).
2. Rank 1 must be DIRE or PESSIMISTIC if ANY stress signals exist (tight deadline, limited resources, unclear scope). If the project looks well-resourced and clearly scoped, rank 1 can be OPTIMISTIC.
3. Goals must be specific and measurable — "Schedule Adherence %" not "Complete on time".
4. Targets and thresholds must be realistic for a project of this type and scale.
5. isLower: true when a lower value is better (cost variance, defect rate, bug count, re-work %).
6. confidence: "high" = project-specific evidence in description, "medium" = industry benchmark, "low" = inferred.
7. evidence_bullets: 2-3 short strings (max 12 words each) explaining what supports this suggestion.
8. context_line: one sentence explaining WHY this specific target makes sense for this project.
9. suggested_action: one specific sentence describing the most important action to take this week.
10. target_value / threshold_value: actual base-unit numbers (e.g., 85 for "85%", 120000 for "120k NOK").`

  const message = await anthropic.messages.create({
    model: 'claude-opus-4-7',
    max_tokens: 4096,
    system: 'You are a precise project risk advisor. Call generate_goal_portfolio with exactly 7 measurable project goals specific to this project.',
