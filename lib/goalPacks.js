...
export function detectIndustry(text, tld, brreg) {
  ...
  // Score-based detection — count keyword hits per industry
  const scores = {
    tech_saas: 0,
    renewable_energy: 0,
    maritime: 0,
  ...
  // Tech/SaaS signals
  if (/\bsaas\b/.test(t)) scores.tech_saas += 4
  if (/\bsoftware\b/.test(t)) scores.tech_saas += 3
  if (/\bplatform\b/.test(t)) scores.tech_saas += 2
  if (/\bapi\b/.test(t)) scores.tech_saas += 2
  if (/\bsubscription\b/.test(t)) scores.tech_saas += 2
  if (/\bcloud\b/.test(t)) scores.tech_saas += 2
  if (/\bapp\b/.test(t)) scores.tech_saas += 1
  if (/programvar|dataprogram/.test(sector)) scores.tech_saas += 5

  // Renewable energy — require ENERGY-specific terms, not just "light" or "power"
  if (/\brenewable\b/.test(t)) scores.renewable_energy += 4
  if (/\bwind (energy|power|turbine|farm)\b/.test(t)) scores.renewable_energy += 4
  if (/\bsolar (energy|power|panel)\b/.test(t)) scores.renewable_energy += 4
  if (/\bhydropower\b|\bhydro power\b|\bwaterkraft\b/.test(t)) scores.renewable_energy += 4
  if (/\bwindmill\b|\bvindkraft\b|\bsolkraft\b/.test(t)) scores.renewable_energy += 4
  if (/\bkraft(verk|produksjon|selskap)\b/.test(t)) scores.renewable_energy += 4
  if (/\bMWh\b|\bGWh\b|\bkWh\b/.test(t)) scores.renewable_energy += 3
  if (/energi(produksjon|selskap|forsyning)|elektrisitet/.test(sector)) scores.renewable_energy += 5

  // Maritime
  if (/\bshipping\b/.test(t)) scores.maritime += 4
  if (/\bvessel\b|\bfleet\b|\boffshore\b/.test(t)) scores.maritime += 3
  if (/\bmaritime\b/.test(t)) scores.maritime += 4
  if (/\bship(ping|yard)?\b/.test(t)) scores.maritime += 3
  if (/sjøfart|maritim|rederi/.test(sector)) scores.maritime += 5

  // Finance
  if (/\bbank(ing)?\b/.test(t)) scores.finance += 3
  if (/\binvestment fund\b|\bhedge fund\b|\bprivate equity\b/.test(t)) scores.finance += 4
  if (/\binsurance\b|\bassurance\b/.test(t)) scores.finance += 3
  if (/\bassets under management\b|\baum\b/.test(t)) scores.finance += 4
  if (/\bfinancial services\b/.test(t)) scores.finance += 3
  if (/\bforsikring\b|\bsparebank\b/.test(t)) scores.finance += 4
  if (/finans(iell)?|bank|forsikring|fond/.test(sector)) scores.finance += 5

  // Retail — include lighting/lamps/furniture/home goods
  if (/\bstore\b|\bshop\b/.test(t)) scores.retail += 3
  if (/\becommerce\b|\be-commerce\b/.test(t)) scores.retail += 4
  if (/\bbutikk\b|\bnettbutikk\b/.test(t)) scores.retail += 4
  if (/\blamp(er|er)?\b|\blampe\b|\blighting\b|\blight(s)?\b|\bbelysning\b/.test(t)) scores.retail += 3
  if (/\bfurniture\b|\bm(ø|o)bel\b|\binteriør\b|\bhome decor\b/.test(t)) scores.retail += 3
  if (/\bproduct(s)?\b.*\bshop\b|\bshopping\b/.test(t)) scores.retail += 2
  if (/\bcart\b|\bcheckout\b|\bbasket\b/.test(t)) scores.retail += 3
  if (/detaljhandel|netthandel|varehandel/.test(sector)) scores.retail += 5

  // Consulting
  if (/\bconsult(ing|ant|ancy)\b/.test(t)) scores.consulting += 4
  if (/\badvisory\b|\bmanagement consulting\b/.test(t)) scores.consulting += 4
  if (/\brådgivning\b|\brådgiver\b/.test(t)) scores.consulting += 4
  if (/konsulent|rådgivning/.test(sector)) scores.consulting += 5

  // Construction
  if (/\bconstruction\b|\bbuilding\b/.test(t)) scores.construction += 3
  if (/\bbygg(e|ing)?\b|\banlegg\b|\bentreprenør\b/.test(t)) scores.construction += 4
  if (/\breal estate\b|\bproperty development\b/.test(t)) scores.construction += 3
  if (/bygg|anlegg|eiendom|konstruksjon/.test(sector)) scores.construction += 5

  // Healthcare
  if (/\bhealthcare\b|\bhealth care\b|\bclinic\b|\bhospital\b/.test(t)) scores.healthcare += 4
  if (/\bpharma(ceutical)?\b|\bmedical\b/.test(t)) scores.healthcare += 3
  if (/\bhelse(tjeneste|foretak|klinikk)\b/.test(t)) scores.healthcare += 4
  if (/helse|medisin|klinikk|sykehus/.test(sector)) scores.healthcare += 5

  // Manufacturing
  if (/\bmanufactur(ing|er)\b/.test(t)) scores.manufacturing += 4
  if (/\bfactory\b|\bproduction plant\b/.test(t)) scores.manufacturing += 3
  if (/\bproduksjon(sanlegg)?\b|\bfabrikk\b/.test(t)) scores.manufacturing += 4
  if (/industri|produksjon|fabrikk/.test(sector)) scores.manufacturing += 5

  // Find highest score
  const best = Object.entries(scores).sort((a, b) => b[1] - a[1])[0]

  // Only use detected industry if score is meaningful (≥3)
