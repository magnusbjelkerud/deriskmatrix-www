...
**Already fixed in Phase 2 commit 1.** The `generateGoalsWithClaude` function now
uses Anthropic tool use with a full JSON Schema definition and
`tool_choice: { type: "tool", name: "generate_goal_portfolio" }`.
This item can be closed when that commit is merged.
